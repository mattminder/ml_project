...
\section*{Results}
The following graphs show the accuracies obtained during cross-validation. The best results were obtained with enhanced data and $\lambda$ smaller than 10\textsuperscript{-3}. There is no significant difference between the performance of logistic regression or support vector machines. 
\begin{figure}[H]
	\centering
...
The fact that train and test accuracies are very similar and equally good for small $\lambda$ shows that overfitting does not occur. It is rather probable that the model underfits and more training iterations could still improve the results. Our submissions on Kaggle however showed that SVM performed better on the test set.
