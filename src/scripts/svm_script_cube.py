...
tx = np.column_stack((tx, tx[:, 0:29]**2, tx[:, 0:29]**3))
...
final_fit, loss = svm_classification(y, tx, best_lambda, np.zeros([tx.shape[1]]), 5000000, 0.001)
...
create_csv_submission(range(350000, 350000+test_preds.size), test_preds,
                      "../../submission/SVM_on_imputed_w_cube.csv")
